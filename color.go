...
// Clear removes all recognized ANSI color codes from the given string and returns the cleaned string.
...
// Set applies the specified Color if valid and prints its string representation.
// Returns an error if the Color is invalid.
func Set(color Color) error {
	//if !ColorLookup.IsValid(color.String()) {
	//	return ErrColorNotFound
	//}
...
func Reset() {
	fmt.Print(resetHelper().String())
}

func resetHelper() Color {
	return ResetColor
}
